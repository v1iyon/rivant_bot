...
export async function addQueueIdeas(ideas) {
  const db = getDb();
  // Параллельно, а не по одной штуке за раз — 20-25 идей последовательными
  ...
  // иногда обрывалась целиком (см. комментарий в lib/claude.js про
  // TIMEOUT_MS). Turso спокойно выдерживает параллельные запросы.
  await Promise.all(
    ideas.map((idea) =>
      db.execute({
...
export async function getPendingPostsDueForMetrics(hoursThreshold) {
  const db = getDb();
  const res = await db.execute({
    sql: `SELECT * FROM pending_posts WHERE metrics_requested = 0 AND posted_at <= datetime('now', ?)`,
