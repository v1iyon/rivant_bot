...
export async function addQueueIdeas(ideas) {
  ...
  for (const idea of ideas) {
    await db.execute({
      sql: `INSERT INTO content_queue (topic, angle, format, suggested_slot, day_of_week, reasoning)
            VALUES (?, ?, ?, ?, ?, ?)`,
      args: [idea.topic, idea.angle, idea.format, idea.suggested_slot, idea.day_of_week || null, idea.reasoning],
    });
  }
}
