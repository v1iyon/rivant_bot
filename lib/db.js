...
export async function getIdeaForHour(hour) {
  const db = getDb();
  // hour — число 0-23, ищем идею, чей suggested_slot начинается с этого часа
  const hourStr = String(hour).padStart(2, "0");
  const res = await db.execute({
    sql: `SELECT * FROM content_queue WHERE status = 'queued' AND suggested_slot LIKE ? ORDER BY id LIMIT 1`,
    args: [`${hourStr}:%`],
  });
  return res.rows[0] || null;
}
