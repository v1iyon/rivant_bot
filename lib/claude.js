...
const TIMEOUT_MS = 42000; // ВАЖНО: это не единственное, что тратит время в
// 60-секундном окне Vercel (Hobby) — после ответа Claude ещё идёт запись
// 20-25 идей в базу (см. lib/db.js addQueueIdeas, теперь параллельно) и
// отправка сообщения в Telegram. Ставили 55 сек — не хватало запаса, вся
// функция обрывалась целиком по хард-лимиту Vercel ДО того, как успевал
// сработать даже catch с сообщением об ошибке — то есть в Telegram
// приходила полная тишина вместо плана ИЛИ понятной ошибки. 42 сек даёт
// Claude достаточно времени (план на Haiku укладывается в 20-30 сек) и
// оставляет ~15+ сек запаса на всё остальное.
