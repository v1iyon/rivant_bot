...
const TIMEOUT_MS = 30000; // если Claude API завис — не ждём вечно, освобождаем cron-вызов
...
// Один повтор при таймауте, сетевом сбое или 5xx (проблема на стороне API,
// не в самом запросе) — раньше единичный сбой сети означал, что напоминание
// или отчёт просто не приходили без объяснений.
export async function askClaude({ system, userMessage, maxTokens = 1500 }) {
  try {
    ...
    if (!isTimeout && !isServerError && !isNetworkError) throw err;
