...
const TIMEOUT_MS = 45000; // таймаут по умолчанию: одна попытка, с запасом под 60-секундный лимит
// Vercel (Hobby) — остальные ~15 сек уходят на БД/графики/отправку в
// Telegram. Длинные ответы (maxTokens до 3000 для "Идей") на Haiku и в
// норме могут генерироваться 20-30+ сек — это не сбой, а просто объём
// текста, поэтому таймаут даём щедрый.
//
// Для очень длинных задач (например, недельный план на 8000 токенов) можно
// передать свой timeoutMs в askClaude — он всё равно должен быть меньше
// maxDuration функции в vercel.json.

async function callClaudeOnce({ system, userMessage, maxTokens, timeoutMs }) {
...
// первая попытка не уложилась в timeoutMs, вторая с тем же лимитом почти
...
// понятную ошибку.
export async function askClaude({ system, userMessage, maxTokens = 1500, timeoutMs = TIMEOUT_MS }) {
  try {
    return await callClaudeOnce({ system, userMessage, maxTokens, timeoutMs });
  } catch (err) {
